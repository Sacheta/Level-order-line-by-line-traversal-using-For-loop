//Program generating a whole binary tree at run time. Supporting Level order traversal line by line using efficient solution of complexity O(n) and auxiliary space of  O(w+h),w=width of tree and h is height. It uses queue DS to implement the required traversal. we push a node into queue. while removing it, we add its left and right children too.So, by te end of one level, next level is already in the queue. after every level we push a null ptr too to getta know the end of that level. and whenever a null ptr is encountered, we print newline and push another null ptr in the queue for next level
...
void node::levelOrder(node *root){  //function for level order traversal of queue
    ...
    //q.push(nullptr); //push null ptr to account end of 1st level
    while(!q.empty()){  //while size of queue is greater than 1. only a single null is left in the end
        int count=q.size();
