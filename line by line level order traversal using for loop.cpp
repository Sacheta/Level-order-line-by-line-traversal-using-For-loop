//Program generating a whole binary tree at run time. Supporting Level order traversal line by line using efficient solution of complexity O(n) and auxiliary space of  O(w),w=width of tree. It uses queue DS to implement the required traversal. we push a node into queue. while removing it, we add its left and right children too.So, by te end of one level, next level is already in the queue. In this method of line by line traversal, we make use of for loop. we enter in a loop for every level and print new line after every for loop
#include<iostream>
#include<queue>
using namespace std;
...
void node::levelOrder(node *root){  //function for level order traversal of queue
    ...
    while(!q.empty()){  //while queue is not empty
        int count=q.size();  //storing current size of queue in count
        for(int i=0;i<count;++i){  //loop printing one level
            node *curr=q.front();
            q.pop();
            cout<<curr->data<<"\t";  //printing current's data
            if(curr->left!=nullptr)  //if left child is not empty
                q.push(curr->left);
            if(curr->right!=nullptr)  //if right child is not empty
                q.push(curr->right);
        }
        cout<<endl;  //print new line after every loop or level
    }
}
